...
async function runTests() {
  ...
  try {
    ...
    if (await signInButton.isVisible()) {
      ...
      const hasValidationError = await page.locator('text=/error|required|please fill/i').count() > 0;
      ...
      await page.screenshot({ path: screenshotPath });
      assert('empty-login', false, 'Sign In button to be present', 'button not found', screenshotPath);
    }
    ...
    if (await submitBtn.isVisible()) {
      ...
      // Check if button was disabled or if there's debouncing
      ...
      results.skipped.push({ testId: 'rapid-submit', reason: 'Sign In button not found' });
      console.log('STEP_SKIP|rapid-submit|Sign In button not found');
    }
    ...
    // Test 10: browser-back (moved to Group B)
    ...
    // First, we need to log in to access SSO page
    console.log('Logging in to access SSO page...');
    await page.goto('http://localhost:8080', { waitUntil: 'networkidle' });
    await page.waitForTimeout(1000);

    // Try to find and fill login form with valid credentials
    const loginUsername = await page.locator('input[type="text"], input[name="username"], input[name="email"]').first();
    ...
    if (await addConfigButton.isVisible()) {
      ...
      const submitButton = await page.locator('button:has-text("保存"), button:has-text("Submit"), button:has-text("Save")').first();
      if (await submitButton.isVisible()) {
        await submitButton.click();
        await page.waitForTimeout(500);

        const hasValidationError = await page.locator('text=/required|error|please fill/i').count() > 0;
    ...
    const closeModal = await page.locator('button:has-text("×"), button:has-text("Close"), button[aria-label="Close"]').first();
    if (await closeModal.isVisible()) {
      await closeModal.click();
      await page.waitForTimeout(500);
    }

    const addConfigButton2 = await page.locator('button:has-text("添加配置"), button:has-text("Add Config"), button:has-text("Add")').first();
    ...
    if (await addConfigButton2.isVisible()) {
      ...
      const endpointInput = await page.locator('input[name*="endpoint"], input[name*="url"], input[placeholder*="endpoint"], input[placeholder*="URL"]').first();

      if (await endpointInput.isVisible()) {
        await endpointInput.fill('not-a-url');

        const submitButton = await page.locator('button:has-text("Submit"), button:has-text("Save"), button:has-text("保存")').first();
        if (await submitButton.isVisible()) {
          await submitButton.click();
          await page.waitForTimeout(500);

          const hasUrlError = await page.locator('text=/url|invalid|endpoint/i').count() > 0;
        ...
        results.skipped.push({ testId: 'sso-invalid-url', reason: 'Endpoint input not found' });
        console.log('STEP_SKIP|sso-invalid-url|Endpoint input not found');
      }
    } else {
    ...
    const toggleButton = await page.locator('button:has-text("启用"), button:has-text("禁用"), button:has-text("Enable"), button:has-text("Disable")').first();
    ...
    const deleteButton = await page.locator('button:has-text("删除"), button:has-text("Delete")').first();
    ...
    if (await deleteButton.isVisible()) {
      ...
      const cancelButton = await page.locator('button:has-text("Cancel"), button:has-text("取消")').first();
