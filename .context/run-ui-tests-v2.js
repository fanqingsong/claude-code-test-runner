  ...
  try {
    ...
    if (await addConfigButton.isVisible()) {
      ...
      if (await submitButton.isVisible()) {
        ...
        const hasValidationError = await page.locator('text=/required|error|请填写/i').count() > 0;
    ...
    if (await addConfigButton2.isVisible()) {
      ...
      // Try to find any input that might be for endpoint
      ...
      for (const input of allInputs) {
        ...
        if (placeholder && placeholder.toLowerCase().includes('endpoint') ||
            name && name.toLowerCase().includes('endpoint') ||
