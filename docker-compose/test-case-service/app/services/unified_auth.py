    ...
    if _casdoor_sdk is None:
        ...
        if not client_id or not client_secret:
            raise HTTPException(
                ...
                detail="Casdoor configuration missing. Set CASDOOR_CLIENT_ID and CASDOOR_CLIENT_SECRET"
            )
...
async def verify_permission(required_roles: list = None):
    ...
    async def dependency(user: dict = Depends(verify_token)):
        ...
        # For local auth, check is_admin flag from database
        if user.get("provider") == "local":
            # We'll need to fetch the user from database to check is_admin
            # This will be done in the endpoint itself
            return user

        # For Casdoor auth, check roles from token
